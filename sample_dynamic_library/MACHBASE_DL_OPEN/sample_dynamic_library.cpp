...
int connectDB()
{
        ...
        )
    {
        ...
        printError(gEnv, gCon, NULL, (char*)"SQLDriverConnect error");
...
void disconnectDB()
{
    ...
    {
        printError(gEnv, gCon, NULL, (char*)"SQLDisconnect error");
    }
...
int executeDirectSQL(const char* aSQL, int aErrIgnore)
{
    ...
    if (gMachApi.mAllocStmt(gCon, &sStmt) != SQL_SUCCESS)
    {
        if (aErrIgnore == 0)
        {
            printError(gEnv, gCon, sStmt, (char*)"SQLAllocStmt Error");
            return RC_FAILURE;
        }
    }
    ...
    if (gMachApi.mExecDirect(sStmt, (SQLCHAR*)aSQL, SQL_NTS) != SQL_SUCCESS)
    {
        ...
        if (aErrIgnore == 0)
        {
            printError(gEnv, gCon, sStmt, (char*)"SQLExecDirect Error");
...
int appendOpen(SQLHSTMT aStmt)
{
    ...
    {
        printError(gEnv, gCon, aStmt, (char*)"SQLAppendOpen Error");
        return RC_FAILURE;
    }
...
int appendData(SQLHSTMT aStmt)
{
    ...
    sParam[5].mDateTime.mDateStr = (char*)"23/May/2014:17:41:28";
    sParam[5].mDateTime.mFormatStr = (char*)"DD/MON/YYYY:HH24:MI:SS";
    sRC = gMachApi.mAppendDataV2(aStmt, sParam);
    CHECK_APPEND_RESULT(sRC, gEnv, gCon, aStmt);
    ...
    *(unsigned int*)(sParam[7].mIP.mAddr) = inet_addr("192.168.0.1");
    ...
    sParam[7].mIP.mAddrString = (char*)"203.212.222.111";
    sRC = gMachApi.mAppendDataV2(aStmt, sParam);
    CHECK_APPEND_RESULT(sRC, gEnv, gCon, aStmt);
...
SQLBIGINT appendClose(SQLHSTMT aStmt)
{
    ...
    {
        printError(gEnv, gCon, aStmt, (char*)"SQLAppendClose Error");
        return RC_FAILURE;
    }
...
int main()
{
    ...
    {
        printError(gEnv, gCon, sStmt, (char*)"SQLAppendSetErrorCallback Error");
        goto error;
    }
